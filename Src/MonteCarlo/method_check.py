...
def calculate_energy_by_bias(ints, bias):
    ...
    m = float(bias.split('_')[2])
    b_type = bias.split('_')[3]
    min_score, max_score = NORM_CONST.loc[(NORM_CONST.bias==b)&(NORM_CONST.N==N)&(NORM_CONST.W==w)&(NORM_CONST.M==1),  ['Min', 'Max']].values[0]
    ...
    if b_type == 'A':
        return 1.0 - ((score - min_score) / (max_score - min_score))**m
    else:
        return ((max_score - min_score) / (score - min_score))**m
