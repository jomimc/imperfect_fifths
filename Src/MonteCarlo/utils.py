...
def get_pair_int_clusters(df, w=20):
    clusters = []
    for pi in [ [int(x) for x in y.split(';')] for y in df.pair_ints]:
        pi_inp = np.array([pi, [0]*len(pi)]).T
        cluster = DBSCAN(eps=w, min_samples=1).fit(pi_inp)
        clu_idx = cluster.labels_
        clu_set = sorted(list(set(clu_idx)))
        for clu_id in clu_set:
...
#       if sum([1 for r in ratios if r not in [1., 2., 3., 4.]]):
            ...
            return True
    df['allowed_ratios'] = df.interval.swifter.apply(lambda x: fn(x))
    return df
...
def are_intervals_distinct(df, cent_tol=30.):
    ...
    df['distinct'] = df.interval.swifter.apply(lambda x: fn(x))
    return df
...
def get_scale_energies_exact_ratios(df):
    ...
    df['energy'] = df.interval.swifter.apply(lambda x: fn(x))
    return df
...
#       ratio_term  = np.mean([1./(y-0.5)**3 + np.exp(y) for y in ratios])
    ...
    df['energy'] = df.ratios.swifter.apply(lambda x: fn(x))
    return df
...
def get_ratios_from_ints(df):
    ...
    df['ratios'] = df.interval.swifter.apply(lambda x: fn(x))
    return df
...
def plot_real_vs_derived_ints(df, pair_ints, iMin=100., n=7, weights=[]):
    ...
    b = [[float(y) for y in x.split(';')] for x in df.loc[(df.min_int.swifter.apply(lambda x: x>=iMin))&(df.n_notes==n)&(df.allowed_ratios), 'interval']]
    sns.distplot([float(y) for x in df.loc[(df.allowed_ratios)&(df.n_notes==n)&(df.min_int.swifter.apply(lambda x: x>=iMin)), 'interval'] for y in x.split(';')], bins=100, ax=ax1[0], label='derived')
...
def get_attractors_in_scale(df):
    ...
    df['attractors'] =  df.all_ints.swifter.apply(lambda x: ';'.join([str(get_nearest_attractor(float(y), sc_f, ratios)) for y in x.split(';')]))
    return df
...
def get_harmonic_similarity_score_df(df):
    sc_i, sc_f, ratios, simil  = get_attractors()
    df['harm_sim'] =  df.all_ints.swifter.apply(lambda x: np.mean([get_similarity_of_nearest_attractor(float(y), sc_f, simil) for y in x.split(';')]))
    return df
...
def get_all_ints_from_pair_ints(df):
    def fn(x):
        ints = [int(y) for y in x.split(';')]
        return ';'.join([str(z) for i in range(len(ints)-1) for z in np.cumsum(ints[i:])])
    df['all_ints'] = df.pair_ints.swifter.apply(lambda x: fn(x))
    return df
...
def get_harmonic_similarity_score_extra_notes(df):
    ...
    df['score_en'] = df.str_fmt.swifter.apply(lambda x: fn(x))
    return df
...
def get_harmonic_similarity_score_equal_temp(df):
    ...
    df['score_eq'] = df.str_fmt.swifter.apply(lambda x: fn(x))
    return df
...
def get_harmonic_similarity_score_equal_temp_extra_notes(df):
    ...
    def fn(x):
        a = [int(y) for y in x*2]
        return np.mean(INTERVAL_SCORE[[l-1 for i in range(len(x)) for l in list(np.cumsum(a[i:i+len(x)]))]])
    df['score_eq_en'] = df.str_fmt.swifter.apply(lambda x: fn(x))
    return df
...
def all_notes_used(df):
    df['all_notes'] = df['mask'].swifter.apply(lambda x: a_n_u(x))
    return df

def get_inst_var_score(df):
    df['inst_keys'] = df['mask'].swifter.apply(lambda x: match_scale_with_instrument(x))
    df['inst_score'] = df.inst_keys.swifter.apply(lambda x: sum([int(y) for y in x.split(';')]))
    df['inst_norm'] = df.inst_score.astype(float) / (df.n_notes.astype(float) * 2. + 1.) / 12.
    df['inst_std'] = df.inst_keys.swifter.apply(lambda x: np.std([float(y) for y in x.split(';')])) /  (df.n_notes.astype(float) * 2. + 1.) / 12.
    tmp_df = df.inst_keys + '-' + df.n_notes.astype(str)
    df['inst_75p'] = tmp_df.swifter.apply(lambda x: count_those_over_75p(x))
    df['inst_85p'] = tmp_df.swifter.apply(lambda x: count_those_over_85p(x))
    df['inst_new_score'] = tmp_df.swifter.apply(lambda x: new_score(x))
    return df
...
def reformat_scales_as_mask(df):
    ...
    idx = df.loc[df.Tuning.swifter.apply(lambda x: x not in ['Unique', 'Turkish'])].index
    df.loc[idx, 'mask'] = df.loc[idx, 'Intervals'].swifter.apply(fn)
    return df
...
def get_2grams_dist(pair_ints, dI=10, imin=0, imax=620):
...
def plot_2gram_dist_by_n_notes(pair_ints, dI=10):
    len_s = np.array([len(x) for x in pair_ints])
    pair_ints = np.array(pair_ints)
    fig, ax = plt.subplots(2,3)
    ax = ax.reshape(ax.size)
    for i, n in enumerate([4,5,6,7,8,9]):
        idx = np.where(len_s==n)[0]
        dist = get_2grams_dist(pair_ints[idx], dI=dI)
#       sns.heatmap(dist[::-1], label=str(n), ax=ax[i])
        sns.heatmap(np.log(dist[::-1]+0.1), label=str(n), ax=ax[i])
        ax[i].set_title(str(n))
    plt.show()

def plot_pair_ints_by_n_notes(pair_ints):
    fig, ax = plt.subplots(4,2)
    ax = ax.reshape(ax.size)
    for i, t in enumerate([4,5,6,7,8,9]):
        sns.distplot([y for x in pair_ints for y in x if len(x)==t], bins=120, label=str(t), ax=ax[i])
        ax[i].legend(loc='best')
        sns.distplot([y for x in pair_ints for y in x if len(x)==t], bins=120, label=str(t), ax=ax[-1])
    plt.show()
