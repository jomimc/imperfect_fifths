...
PRO_DIR  = '/home/johnmcbride/projects/Scales/Toy_model/Data/Processed3/'
...
if __name__ == "__main__":
    ...
    biases = ['none'] + [f"TRANS{a}_{b}" for a in ['A', 'B'] for b in range(1,4)] + \
             [f"HAR_{b}_{a}" for a in range(1,4) for b in range(5,25,5)] + \
             [f"{a}_{b}" for a in ['HAR', 'FIF'] for b in range(5,25,5)]

    groups = ['MIN'] + ['TRANS']*6 + ['HAR']*4 + ['HAR2']*4 + ['HAR3']*4 + ['HAR']*4 + ['FIF']*4
    ...
    if args.partabase == 'None':
        df.to_feather(os.path.join(BASE_DIR, 'monte_carlo_comparison3.feather'))
