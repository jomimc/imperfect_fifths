...
PRO_DIR  = '/home/johnmcbride/projects/Scales/Toy_model/Data/Processed4/'
...
#           df_g = df_grid.loc[(df_grid.n_notes==n)&(df_grid.min_int>mi)&(df_grid.max_int<ma)&(df_grid.beta==beta)]
    ...
    biases = [f"HAR_10_{a:4.2f}_{b}" for a in [0.1, 0.25, 0.5, 1, 2, 4, 10] for b in ['A', 'B']]
    biases2 = [f"HAR_10_{a:05.2f}_{b}" for a in [0.1, 0.25, 0.5, 1, 2, 4, 10] for b in ['A', 'B']]

    print(biases)
    groups = ['HAR']*14
    bias_change = {biases[i]:biases2[i] for i in range(len(biases))}
...
#   df = df.reset_index(drop=True) 
    ...
    df['bias'] = df.bias.apply(lambda x: bias_change[x])
    ...
    df.reset_index(drop=True).to_feather(os.path.join(BASE_DIR, 'methods_check.feather'))
